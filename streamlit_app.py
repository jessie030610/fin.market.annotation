...
# Decisions input with dynamic keys
st.subheader("Your Decisions")
buy_selection = st.multiselect("Select companies to BUY", options, key=f"buy_{idx}")
sell_selection = st.multiselect("Select companies to SELL", options, key=f"sell_{idx}")
reason = st.text_area("Briefly describe the reason for your decision", height=100, key=f"reason_{idx}")

# Buttons: Confirm and Next
...
with col1:
    if st.button("Confirm", key=f"confirm_{idx}"):
        ...
        st.success(f"Decision recorded for {task_file}")
with col2:
    if st.button("Next", key=f"next_{idx}"):
        ...
        # State change triggers rerun, clearing widgets due to dynamic keys
